...
| `OBR-PRIV-04` | obrigatório | avaliar e comunicar incidentes aplicáveis | LGPD art. 48; Res. CD/ANPD 15/2024 | runbook 5 de [runbooks.md](runbooks.md); `audit_logs` append-only | parcialmente verificado — simulado não executado |
...
| `OBR-SEC-02` | obrigatório | ciclo seguro de desenvolvimento e vulnerabilidades | PoSIC 7.9-7.10; PPSI controles 7, 16 e 18 | `.github/workflows/ci.yml` com lint, typecheck, testes, build, migrations, pgTAP e advisors; actions e CLI com versão imutável | parcialmente verificado — sem varredura de dependências e sem pentest autorizado |
...
| `OBR-SEC-04` | obrigatório | resposta e registro de incidentes | PoSIC 6.5 e 7.11; PPSI controle 17 | runbook 5 de [runbooks.md](runbooks.md); trilha em `audit_logs` | parcialmente verificado — alerta e plantão dependem de `TBD-STI-05` e `TBD-STI-07` |
...
| `OBR-ACC-01` | obrigatório no escopo adotado | aplicar recomendações eMAG pertinentes e avaliar manualmente | eMAG 3.1, caps. 2-4 | seção 6 de [accessibility.md](accessibility.md); `src/design-tokens.test.ts`; `src/admin/AdminApp.test.tsx` | parcialmente verificado — avaliação manual e com pessoas usuárias pendente (`TBD-ACC-01`) |
