...
| Decidir remoção | ❌ | ✅ | ✅ | `private.can_moderate()` |
...
| Gerir o próprio MFA e encerrar as próprias sessões | ✅ | ✅ | ✅ | Supabase Auth, rota `/admin/security` |
| Alterar `audit_logs` | ❌ | ❌ | ❌ | append-only para a aplicação |
| Acessar `service_role` | ❌ | ❌ | ❌ | exclusivo das Edge Functions |

Restrições adicionais já implementadas: nenhuma pessoa decide o próprio conteúdo; `ADMIN` não altera `SUPERADMIN`; o último `SUPERADMIN` ativo não pode ser removido nem desativado; toda concessão e revogação de papel é auditada; sessão administrativa expira em 60 minutos absolutos.
