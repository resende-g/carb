...
5. `20260904011125_pre_sti_hardening.sql`: revoga sessões administrativas em eventos críticos, impede autodecisão de remoção e vincula paths do Storage a namespace, proprietário e registro editorial.
...
`admin_sessions` guarda apenas `session_id`, usuário e prazos; nunca armazena access token ou refresh token. `profiles.admin_sessions_blocked` impede o uso de autoridade entre a remoção/reset do último fator MFA e a verificação de outro. `supabase/seed.sql` contém somente fixtures públicas ou sintéticas. Os testes pgTAP em `supabase/tests/database/` validam RLS, papéis, MFA, expiração, onboarding, reativação, autoaprovação, proteção de 2–3 SUPERADMINs, integridade, auditoria e o contrato de `dashboard_metrics`.
